...
def stpars(n_ms, n_rg, feh, afe, age, logg_cn = 3, fig = False, iso = 'DARTMOUTH'):
    ...
    if iso.upper() == 'DARTMOUTH': print('Utilizing the DARTMOUTH isochrones')
    if iso.upper() == 'PADOVA':    print('Utilizing the Padova isochrones')
    ...
    for i in range(len(teff_grid)):
        ...
        if i <= index_lim:  xxxx = isologg >= logg_lim
        else:               xxxx = isologg < logg_lim
...
def gettracks(feh, afe, age, iso = 'DARTMOUTH'):
    ...
    os.chdir("../")
    if iso.upper() == 'DARTMOUTH':
        if age < 10: isofileout = "./DARTMOUTH/a0" + str(int(age)*1000) + "isotemp"
        else:        isofileout = "./DARTMOUTH/a" + str(int(age)*1000) + "isotemp"
    
    if iso.upper() == 'PADOVA':
        if age < 10: isofileout = "./DARTMOUTH/isochrones/Padova/iso0" + str(int(age)) + ".dat"
        else:        isofileout = "./DARTMOUTH/isochrones/Padova/iso" + str(int(age)) + ".dat"
...
def set_stpars_filename(n_ms, n_rg, feh, afe, age, logg_cn = 3):
    ...
    if feh < 0: feh_s = "%1s%4.2f" % ('-', abs(feh))
    else:       feh_s = "%1s%4.2f" % ('+', abs(feh))
        
    if afe < 0: afe_s = "%1s%4.2f" % ('-', abs(afe))
    else:       afe_s = "%1s%4.2f" % ('+', abs(afe))
        
    if age < 10: age_s = "%1s%3.1f" % ('0', abs(age))
    else:        age_s = "%4.1f" % abs(age)
        
    if n_ms < 10: n_ms_s = "%1s%1i" % ('0', abs(n_ms))
    else:         n_ms_s = "%2i" % abs(n_ms)
        
    if n_rg < 10: n_rg_s = "%1s%1i" % ('0', abs(n_rg))
    else:         n_rg_s = "%2i" % abs(n_rg)
